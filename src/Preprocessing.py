...
def Remove_Outliers(df):
    ...
    plot_boxplot(df, "Delay")
    print("Removing outliers is finished ")
    return df
...
def Select_Features(df,df_flht):
    new_df=df[['Depature Airport New','Destination Airport New','Delay']].copy()
...
def Splitting_Data(df):
    x_train,x_test,y_train,y_test=train_test_split(df.drop('Delay', axis = 1),df['Delay'],test_size=0.105359,shuffle=False)
